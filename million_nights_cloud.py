...
モード (環境変数 MN_MODE、未指定ならUTC時刻から自動判定):
  ...
  noon    : 11:50 JST 昼のトリガー監視タイム(昼休み11:45〜12:50用)
  evening : 17:15 JST 終値チェック&日誌タイム(夜17:00〜19:00用)
...
def mode_noon():
    ...
    try:
        n = fetch_nikkei()
        line = f"📊 前場の日経平均: {n['icon']} {n['label']} ({n['close']:,.0f}円)"
    except Exception:
        line = ""
    send_discord(
        ...
        "_前場の値動きには飛び乗らない。「トリガー超え」だけが買う理由です。3分で切り上げて午後のお仕事へ。_"
    )
